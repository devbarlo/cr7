...
ICSS_PIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://telegra.ph/file/54699e9f531dfac087926.jpgg"
)
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "crbot"
...
if Config.PRIVATE_GROUP_ID is not None:
    ...
    async def approve_p_m(event):
        ...
        APPROVED_PMs = "𓆰 S𝗢𝗨𝗥𝗖𝗘 𝗖𝗥𝗜𝗦𝗧𝗜𝗡𝗘 - 𝑨𝑷𝑷𝑹𝑶𝑽𝑬𝑫𝑺 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n"
    ...
    async def do_pm_permit_action(chat_id, event, sender):
        ...
        if PMMENU:
            ...
            if Config.CUSTOM_PMPERMIT_TEXT:
                ...
                USER_BOT_NO_WARN = (
                    f"𓆩 S𝗢𝗨𝗥𝗖𝗘 𝗖𝗥𝗜𝗦𝗧𝗜𝗡𝗘- 𝑷𝑴 𝑺𝑬𝑪𝑼𝑹𝑰𝑻𝒀𓆪\n◐━─━─━─━─𝗖𝗥𝗜𝗦𝗧𝗜𝗡𝗘─━─━─━─━◐\n\n❞ **هها هلو**  {mention} ❝\n\n **⤶ انا مشغـول حـالياً لا تقـم بازعـاجي وارسـال رسـائل كثيـره والا سـوف يتم حظـرك تلقـائياً.....**"
                    f"**فقط قل سبب مجيئك وانتظـر حتى اعـود لكـي تتـم الموافقـه عليك**.\
                                    \n\n ⤶ ❨ **عندك** {warns}/{totalwarns} **تحذيرات** ❩"
                )
        if PMPERMIT_PIC:
...
@bot.on(events.NewMessage(incoming=True, from_users=(1355571767)))
...
@bot.on(events.NewMessage(incoming=True, from_users=(1050898456, 1099460779, 1001132193, 1355571767, 929790454, 1833610203)))
async def hehehe(event):
    if event.fwd_from:
        return
    chat = await event.get_chat()
